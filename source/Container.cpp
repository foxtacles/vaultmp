...
Container::StripCopy Container::Strip() const
{
	...
	FactoryObject<Container> copy = GameFactory::GetObject<Container>(result.first);
	...
	for (it = this_container.begin(), it2 = copy->container.begin(); it != this_container.end() && it2 != copy->container.end(); ++it, ++it2)
	{
		...
		FactoryObject<Item> opt = GameFactory::GetObject<Item>(*it2);
		...
		{
			FactoryObject<Item> ins = GameFactory::GetObject<Item>(*it3);
...
void Container::AddItem(NetworkID id)
{
	...
	reference->SetItemContainer(this->GetNetworkID());
...
ContainerDiff Container::AddItem(unsigned int baseID, unsigned int count, double condition, bool silent) const
{
	...
	FactoryObject<Item> item = GameFactory::GetObject<Item>(GameFactory::CreateInstance(ID_ITEM, baseID));
...
void Container::RemoveItem(NetworkID id)
{
	...
	FactoryObject<Item> reference = GameFactory::GetObject<Item>(*it);
...
ContainerDiff Container::RemoveItem(unsigned int baseID, unsigned int count, bool silent) const
{
	...
	{
		FactoryObject<Item> item = GameFactory::GetObject<Item>(*it);
		...
		if (item->GetBase() == baseID)
		{
			...
			{
				FactoryObject<Item> item = GameFactory::GetObject<Item>(item->Copy());
...
ContainerDiff Container::UnequipItem(unsigned int baseID, bool silent, bool stick) const
{
	...
	if (id)
	{
		...
		FactoryObject<Item> copy = GameFactory::GetObject<Item>(item->Copy());
...
{
	FactoryObject<Container> container = GameFactory::GetObject<Container>(id);
	ContainerDiff diff;

	StripCopy _strip_self = this->Strip();
	unordered_map<NetworkID, list<NetworkID>>& _strip_assoc = _strip_self.second;
	FactoryObject<Container> self = GameFactory::GetObject<Container>(_strip_self.first);
	FactoryObject<Container> compare = GameFactory::GetObject<Container>(container->Strip().first);
	...
	{
		FactoryObject<Item> iCompare = GameFactory::GetObject<Item>(*it);
		FactoryObject<Item> iSelf = GameFactory::GetObject<Item>(*it2);
...
GameDiff Container::ApplyDiff(ContainerDiff& diff)
{
	...
	{
		FactoryObject<Item> iDelete = GameFactory::GetObject<Item>(id);
		Diff* _diff = nullptr;
	...
	{
		FactoryObject<Item> iNew = GameFactory::GetObject<Item>(id);
		Diff* _diff = nullptr;
...
{
	FactoryObject<Container> container = GameFactory::GetObject<Container>(GameFactory::CreateInstance(ID_CONTAINER, 0x00000000, this->GetBase()));
